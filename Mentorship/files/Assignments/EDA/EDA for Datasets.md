...
#### Download a dataset
Step 1:
Naviagte to the NOAA website via this link:
+ https://www.ncei.noaa.gov/metadata/geoportal/rest/metadata/item/gov.noaa.ncdc:C00946/html

Step 2:
Click on the first link in the portal under "Search Data" section titled: "NCEI Data Search"
![image](https://github.com/user-attachments/assets/655b01d0-d1f0-4afb-ac77-bcf234446c5b)

Step 3: 
From there enter some search criteria to test and download only one of the files (preferably the first on the list) to test.
...
#### Preview the data
Step 5:
Now that you have done your imports, let's read in the data:
+ data_dir = "path to data/datafile.csv"
+ data = pd.read_csv(f"{data_dir}")

Step 6:
What is in our data? This can be answered several ways. For a short preview we can look into the size, small extract of the data and basic info. Let's look at each one in order.
+ Size of the data
++ print(data.size)
...
+ Basic data info
++ data.info()
  The .info() method in Pandas provides a concise summary of a DataFrame. When executed, it prints information about the DataFrame's structure, including: 
...
This method is useful for quickly assessing the overall structure of a DataFrame, identifying potential data quality issues like missing values, and understanding memory consumption.
...
For numerical columns, the output includes:
...
For object data types (e.g., strings or mixed types), the output includes:
count: The number of non-null values.
